...
const stripePromise = loadStripe(process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY || 'pk_test_51RlV0ULm4wWgyhyBvv0KqRNtKgvbBZVv1Z0uwHhygxjbiXmRuEWic2jFgR8TgcyoeGmWVsZWwWlSxYPEXPflA9o300udbmpj4W');
...
const CheckoutForm = () => {
  ...
  const fetchSavedCards = async () => {
    try {
      console.log('Attempting to fetch saved cards from:', `${API_BASE_URL}/api/cards`);
      ...
      if (response.ok) {
        ...
        console.log('Saved cards data received:', data);
        setSavedCards(data.cards || []);
      } else {
        console.error('Failed to fetch saved cards - Status:', response.status);
        const errorText = await response.text();
        console.error('Error response body:', errorText);
      }
    } catch (error) {
      console.error('Network error fetching saved cards:', error);
      if (error instanceof Error) {
        ...
        // Set a user-friendly message for connection issues
        if (error.name === 'TypeError' && error.message.includes('fetch')) {
          console.error('Backend server may not be running on port 2000');
        }
      }
    } finally {
  ...
  const handleRemoveCard = async (cardId: string) => {
    try {
      ...
      if (error instanceof Error) {
        console.error('Remove card error details:', {
          name: error.name,
          message: error.message
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const paymentIntentResponse = await fetch(`${API_BASE_URL}/api/createPaymentIntent`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (paymentIntent.status === 'succeeded') {
        const confirmResponse = await fetch(`${API_BASE_URL}/api/confirmPayment`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      if (error instanceof Error) {
        ...
        if (error.name === 'TypeError' && error.message.includes('fetch')) {
          setError('Unable to connect to payment server. Please check if the backend is running on port 2000.');
